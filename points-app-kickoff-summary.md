...
- **Not yet done:** actually adding it to Jess's and Abby's home screens and confirming the icon/standalone launch experience looks right on a real device — the `apple-touch-icon` and `apple-mobile-web-app-*` meta tags are in place and unblocked now that there's a real URL, just not device-tested yet.
